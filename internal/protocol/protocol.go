...
type Encoder func(map[uint32][]byte) ([]byte, error)

type Decoder func([]byte) (map[uint32][]byte, error)
...
func Encode(msg map[uint32][]byte) ([]byte, error) {
	return EncodeUnsafe(msg, sort.Sort, true)
}

func EncodeUnsafe(msg map[uint32][]byte, sort func(sort.Interface), checkLen bool) ([]byte, error) {
	if len(msg) == 0 {
...
func CreateRequest(encode Encoder, rand io.Reader, prevReply []byte) (nonce, blind [NonceSize]byte, request []byte, err error) {
...
func CreateReplies(encode Encoder, nonces [][]byte, midpoint uint64, radius uint32, cert []byte, privateKey []byte) ([][]byte, error) {
	if len(nonces) == 0 {
		return nil, nil
	}
...
func CreateCertificate(encode Encoder, minTime, maxTime uint64, publicKey, rootPrivateKey []byte) (certBytes []byte, err error) {
	if maxTime < minTime {
...
func getSubmessage(decoder Decoder, msg map[uint32][]byte, tag uint32, name string) (result map[uint32][]byte, err error) {
	valueBytes, err := getValue(msg, tag, name)
	if err != nil {
		return nil, err
	}

	result, err = decoder(valueBytes)
	if err != nil {
...
func VerifyReply(decode Decoder, replyBytes, publicKey []byte, nonce [NonceSize]byte) (time uint64, radius uint32, err error) {
	reply, err := decode(replyBytes)
	if err != nil {
		return 0, 0, errors.New("protocol: failed to parse top-level reply: " + err.Error())
	}

	cert, err := getSubmessage(decode, reply, TagCERT, "certificate")
	if err != nil {
		return 0, 0, err
	}
