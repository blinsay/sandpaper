...
func TestRequestSize(t *testing.T) {
	_, _, request, err := CreateRequest(Encode, rand.Reader, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func createServerIdentity(t *testing.T) (cert, rootPublicKey, onlinePrivateKey []byte) {
	...
	if cert, err = CreateCertificate(Encode, 0, 100, onlinePublicKey, rootPrivateKey); err != nil {
		t.Fatal(err)
	}
...
func TestRoundtrip(t *testing.T) {
	...
	for _, numRequests := range []int{1, 2, 3, 4, 5, 15, 16, 17} {
		...
		replies, err := CreateReplies(Encode, noncesSlice, expectedMidpoint, expectedRadius, cert, onlinePrivateKey)
		if err != nil {
			t.Fatal(err)
		}

		for i, reply := range replies {
			midpoint, radius, err := VerifyReply(Decode, reply, rootPublicKey, nonces[i])
			if err != nil {
...
func TestChaining(t *testing.T) {
	...
	nonce1, _, _, err := CreateRequest(Encode, rand.Reader, nil)
	if err != nil {
		t.Fatal(err)
	}

	replies1, err := CreateReplies(Encode, [][]byte{nonce1[:]}, 10, 0, certA, onlinePrivateKeyA)
	if err != nil {
		t.Fatal(err)
	}

	nonce2, blind2, _, err := CreateRequest(Encode, rand.Reader, replies1[0])
	if err != nil {
		t.Fatal(err)
	}

	replies2, err := CreateReplies(Encode, [][]byte{nonce2[:]}, 5, 0, certB, onlinePrivateKeyB)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, step := range claim {
		...
		midpoint, _, err := VerifyReply(Decode, step.reply, step.serverPublicKey, nonce)
		if err != nil {
			t.Fatal(err)
		}
